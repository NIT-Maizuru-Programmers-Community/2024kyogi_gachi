...
class simu(judge.Judgec,algorithm_general.algorithm_tentative,algorithm.algorithm_tentative,algorithm_tate_ippan.algorithm_tentative,task_kill.algorithm_kill):
    ...
    def relord_judge_log(self):
        ...
        self.call_algotithm=self.kill(self.now_board,self.correct_board,self.use_type,self.wide,self.height)#アルゴリズム呼び出し,
        self.end_time = time.time()#終了時間
        self.time=self.end_time-self.start_time#かかった時間

        print(f"killは{len(self.call_algotithm)}手かかりました")
